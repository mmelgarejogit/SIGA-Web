...
import {
  type TipoLente,
  type CreateTipoLenteRequest,
  type UpdateTipoLenteRequest,
  getTiposLente,
  createTipoLente,
  updateTipoLente,
  deactivateTipoLente,
...
async function load() {
  ...
  loadError.value = ""
  try {
    items.value = await getTiposLente()
  } catch (err: unknown) {
    loadError.value = err instanceof Error ? err.message : "Error al cargar tipos de lente."
  } finally {
...
function menuItems(item: TipoLente): ContextMenuItem[] {
  return [
...
async function submitCreate() {
  ...
  try {
    ...
    createError.value = err instanceof Error ? err.message : "Error al crear tipo de lente."
  } finally {
...
async function submitEdit() {
  ...
  try {
    ...
    editError.value = err instanceof Error ? err.message : "Error al actualizar tipo de lente."
  } finally {
...
const deactivatingItem = ref<TipoLente | null>(null)

function openDeactivate(item: TipoLente) {
...
async function confirmDeactivate() {
  ...
  deactivateError.value = ""
  try {
    await deactivateTipoLente(deactivatingItem.value.id)
...
<template>
  <div class="min-h-screen" style="background-color: var(--color-background)">
            ...
            <template #nombre="{ item }">
              <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style="background-color: var(--color-primary-container)">
                  <span class="material-symbols-outlined" style="font-size: 18px; color: white">lens</span>
                </div>
    ...
    <!-- Modal Crear -->
    <BaseModal :show="showCreate" title="Nuevo Tipo de Lente" size="lg" @close="showCreate = false">
      ...
      <template #footer>
        ...
        <BaseButton variant="primary" :disabled="isSaving" @click="submitCreate">
          {{ isSaving ? "Creando…" : "Crear tipo" }}
        </BaseButton>
      </template>
    </BaseModal>

    <!-- Modal Editar -->
    <BaseModal :show="showEdit" title="Editar Tipo de Lente" size="lg" @close="showEdit = false">
      ...
      <div class="space-y-4">
        ...
        <div class="flex items-center gap-3 p-4 rounded-xl" style="background-color: var(--color-surface-container-low)">
          ...
          <label for="editIsActive" class="text-sm font-medium cursor-pointer" style="color: var(--color-on-surface)">Tipo activo</label>
        </div>
      </div>
      <template #footer>
    ...
    <!-- Modal Desactivar -->
    <BaseModal :show="showDeactivate" title="Desactivar Tipo de Lente" size="sm" @close="showDeactivate = false">
