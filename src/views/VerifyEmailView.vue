...
<template>
  <main
    ...
    style="background-color: #F7F9FE; font-family: 'Manrope', system-ui, sans-serif;"
  >
    <div
    ...
    >
      <!-- Brand -->
      <p
        class="text-2xl font-black tracking-tight mb-10"
      ...
      <div v-if="status === 'loading'" class="flex flex-col items-center gap-4">
        <svg class="animate-spin w-10 h-10" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          <circle class="opacity-25" cx="12" cy="12" r="10" stroke="#00288E" stroke-width="4" />
          <path class="opacity-75" fill="#00288E" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
        </svg>
        <p class="font-medium" style="color: #444653;">Verificando tu email...</p>
      </div>
      ...
      <div v-else-if="status === 'success'" class="flex flex-col items-center gap-4">
        ...
        <RouterLink
          ...
          style="background-color: #00288E; color: white; box-shadow: 0 8px 24px rgba(0,40,142,0.2);"
        >
          Iniciar sesión
      ...
      <div v-else class="flex flex-col items-center gap-4">
        ...
        >
          <span class="material-symbols-outlined" style="color: #93000A; font-size: 32px;">error</span>
        </div>
        <h2
          class="text-2xl font-bold"
          style="font-family: 'Plus Jakarta Sans', system-ui, sans-serif; color: #181C20;"
        >No se pudo verificar</h2>
        <p class="font-medium" style="color: #444653;">{{ errorMsg }}</p>
        <RouterLink
          to="/login"
          class="mt-4 inline-flex items-center gap-2 h-12 px-8 rounded-full font-bold transition-all"
          style="border: 1.5px solid #C4C5D5; color: #00288E; background: white;"
        >
          Volver al login
        </RouterLink>
      </div>
